...
export default function About() {
  ...
  const passions = [
    { icon: '🤖', title: 'IA & Automatización', desc: 'Fascina construir sistemas inteligentes que automatizan procesos complejos' },
    { icon: '🎨', title: 'Diseño Digital', desc: 'Crear interfaces bellas y funcionales que impacten la experiencia del usuario' },
    { icon: '🚀', title: 'Innovación Tech', desc: 'Explorar nuevas tecnologías y aplicarlas para resolver problemas reales' },
    { icon: '🔒', title: 'Seguridad', desc: 'Desarrollar soluciones seguras y confiables para proteger datos' },
  ];

  return (
    <section id="about" className="py-32 bg-[#0a0a12] relative">
      <div className="max-w-6xl mx-auto px-4">
        ...
        >
          <div className="flex items-center gap-3 mb-4">
            <span className="text-[#00e5ff] font-mono text-sm">01</span>
            <div className="h-px w-8 bg-gradient-to-r from-[#00e5ff] to-transparent" />
            <span className="text-[#c8c8d8] text-sm uppercase tracking-widest">Sobre Mí</span>
          </div>
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-6">
            Ingeniero de Sistemas & Desarrollador Full Stack
          </h2>
          <p className="text-lg text-[#c8c8d8] max-w-2xl leading-relaxed">
            ...
            {skills.map((skill, index) => (
              <div key={index} className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[#00e5ff] text-sm">{skill.level}%</span>
                </div>
                <div className="h-1 bg-[#10101a] rounded-full overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    whileInView={{ width: `${skill.level}%` }}
                    transition={{ duration: 1, delay: index * 0.1 }}
                    className="h-full bg-gradient-to-r from-[#00e5ff] to-[#c77dff] rounded-full"
                  />
                </div>
              </div>
            ))}
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
            {passions.map((passion, index) => (
              <motion.div
                ...
                className="p-6 rounded-lg border border-[#00e5ff]/20 bg-[#10101a]/50 backdrop-blur hover:border-[#00e5ff]/50 hover:bg-[#10101a] transition-all group"
              >
                <div className="text-3xl mb-3 group-hover:scale-110 transition-transform">{passion.icon}</div>
                <h4 className="font-bold text-white mb-2">{passion.title}</h4>
                <p className="text-sm text-[#c8c8d8]">{passion.desc}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
